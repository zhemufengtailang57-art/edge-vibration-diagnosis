"""
最终部署模型训练 - 90%转速单工况
数据: normal/loose/unbalance 各 duty900
特征: 17维 (RMS + 16bin)
输出: mlp_weights_final.npz → ai_model.h
"""
...
import glob
normal = load_csv(glob.glob(r'normal_duty900*.csv')[0], 0)
loose  = load_csv(glob.glob(r'loose_duty900*.csv')[0], 1)
unbal  = load_csv(glob.glob(r'unbalance_duty900*.csv')[0], 2)
...
print(f'\n5折交叉验证: {cv_scores.mean()*100:.1f}% ± {cv_scores.std()*100:.1f}%')
...
np.savez(r'mlp_weights_final.npz',
         ...
         mean=mean, std=std)
n_params = mlp.coefs_[0].size + mlp.intercepts_[0].size + mlp.coefs_[1].size + mlp.intercepts_[1].size + mlp.coefs_[2].size + mlp.intercepts_[2].size
print(f'\n模型: 17->32->16->3, 参数{n_params}个')
